...
def run_sim(run_parameters, verbose=True):
    ...
    infection_lookup = burnin_starting_infections(human_lookup)
    ...
    for t in range(transmission_burnin_period):
        new_state = evolve(human_lookup=human_lookup,
        ...
        infection_lookup = new_state["infection_lookup"]
        vector_lookup = new_state["vector_lookup"]
        infection_barcodes = new_state["infection_barcodes"]
        vector_barcodes = new_state["vector_barcodes"]
        root_genotypes = new_state["root_genotypes"]
        previous_max_infection_id = new_state["previous_max_infection_id"]
        ...
        new_state_summary = pd.DataFrame({"time": [t+1],
                                            ...
                                            "n_unique_genotypes": count_unique_barcodes(new_state["infection_lookup"]),
        ...
        if t > 30 and immunity_on:
            ...
            mean_daily_eir = summary_statistics["n_infected_vectors"].iloc[-10:].mean()
