...
def post_process_simulation(final_sim_state,
    ...
    if allele_freq_plot:
        try:
            ...
            print("MemoryError: Skipping allele frequency plot")
    ...
    if ibx_plot:
        try:
            ...
            print("MemoryError: Skipping IBX plot")
    ...
    if within_host_ibx_plot:
        try:
            ...
            if track_roots:
                ...
                axs[0, 0].set_xlabel("Complexity of Infection")
                axs[0, 0].set_ylabel("Density")
                axs[1, 1].hist(ibd_values, bins=50, density=True, orientation="horizontal")
                axs[1, 1].set_xlabel("Density")
                axs[1, 1].set_ylabel("Within-host IBD")
                axs[1, 0].scatter(coi_values, ibd_values, alpha=0.1)
                axs[1, 0].set_xlabel("Complexity of Infection")
                axs[1, 0].set_ylabel("Within-host IBD")
                plt.tight_layout()
                plt.savefig(f"within_host_ibd.png")
        except MemoryError:
            print("MemoryError: Skipping within-host IBX plot")
